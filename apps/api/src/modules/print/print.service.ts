...
export interface PricePrintLine { menge: number; bezeichnung: string; einzelpreisCents: number; listenpreisCents?: number | null; rabattPct?: number | null; artNr?: string; detail?: string[]; alternativ?: boolean; platzierung?: string; altPreisText?: string; imPdfAusblenden?: boolean; lineType?: import("@texma/shared").LineType; staffel?: Array<{ abMenge: number; vkCents: number }> }
...
export interface PrintRepository {
  ...
  /** Original-Rechnungs-ID eines Mahnbelegs — für den Mitversand der Rechnung; null wenn unbekannt. */
  invoiceIdForNotice(id: string): Promise<string | null>;
...
export class PrintService {
  ...
  /** Original-Rechnungs-ID eines Mahnbelegs (für den Mitversand der Rechnung). */
  invoiceIdForNotice(id: string): Promise<string | null> {
    return this.repo.invoiceIdForNotice(id);
  }
