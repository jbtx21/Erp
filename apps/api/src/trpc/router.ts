...
export const appRouter = router({
  ...
  sales: router({
    createOrder: roleProcedure("ADMIN", "BUERO")
      .input(z.object({
        companyId: z.string().min(1),
        lines: z.array(z.object({ description: z.string().min(1), qty: z.number().int().positive(), unitNetCents: z.number().int().min(0), listNetCents: z.number().int().min(0).optional(), rabattPct: z.number().int().min(0).max(100).optional(), taxRatePct: z.number().int().min(0).max(100).optional(), kind: z.enum(["TEXTIL", "VEREDELUNG", "SONSTIGE"]).optional(), variantId: z.string().optional(), bezugPositionen: z.array(z.number().int().positive()).optional(), dbCents: z.number().int().optional(), lineType: z.enum(["ARTIKEL", "GRUPPE", "ZWISCHENSUMME", "GRUPPENSUMME"]).optional(), placement: z.string().max(120).optional(), motiv: z.string().max(120).optional(), motivGroesse: z.string().max(60).optional(), farbton: z.string().max(80).optional(), platzierungsdetails: z.string().max(200).optional(), sonstiges: z.string().max(200).optional(), altPreisText: z.string().max(120).optional(), imPdfAusblenden: z.boolean().optional() })).min(1),
      }))
      .mutation(async ({ input, ctx }) => {
    ...
    update: roleProcedure(...supplierRoles)
      .input(z.object({
        ...
        lines: z.array(z.object({
          ...
          placement: z.string().max(120).nullable().optional(), motiv: z.string().max(120).nullable().optional(), motivGroesse: z.string().max(60).nullable().optional(), farbton: z.string().max(80).nullable().optional(), platzierungsdetails: z.string().max(200).nullable().optional(), sonstiges: z.string().max(200).nullable().optional(), 
