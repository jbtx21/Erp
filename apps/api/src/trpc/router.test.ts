...
function setup(user: AuthUser | null = BUERO) {
  ...
  const ctx: Context = {
    ...
    gutscheine: { list: async () => [], create: async () => ({}), redeem: async () => ({ appliedCents: 0, remainingCents: 0 }) } as unknown as Context["gutscheine"],
    apiTokens: { list: async () => [], create: async () => ({ token: "", record: {} }), revoke: async () => undefined, verify: async () => null } as unknown as Context["apiTokens"],
    ...
    matrixImport: new MatrixImportService(new InMemoryMatrixImportRepository(), new MemoryAuditSink()),
...
describe("tRPC RBAC — Produktion ohne Preis-/Kundenzugriff (Kap. 12)", () => {
  ...
  it("PRODUKTION erhält redigierten Preis (null), Kundenvermerk bleibt sichtbar", async () => {
    ...
    const prod = createCallerFactory(appRouter)({
      ...
      statusAmpel: {} as Context["statusAmpel"],
      sammelbestellung: {} as Context["sammelbestellung"],
      gutscheine: {} as Context["gutscheine"],
      apiTokens: {} as Context["apiTokens"],
      ...
      matrixImport: {} as Context["matrixImport"],
      auth: {} as Context["auth"],
      user: PRODUKTION,
      sessionToken: "tok",
      setSessionCookie: vi.fn(),
